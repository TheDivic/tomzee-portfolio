...
- **Film grain.** A very faint fixed texture over the whole page (see
  ...
  nothing.
- **The designer's seal.** On the home hero, a slow-spinning ring of text that
  behaves like a watermark/stamp. It's `aria-hidden` decoration — if it ever
  becomes annoying, delete the `.home-hero__stamp` div from `index.html`.
...
one-property change in the relevant `::before` rule; the stamp's spin speed is
`animation: seal-spin 32s` in `.home-hero__stamp`.
